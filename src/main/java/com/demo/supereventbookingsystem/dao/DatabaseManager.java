...
public class DatabaseManager {
    ...
    private void createTables() throws SQLException {
        ...
        String createEventsTable = "CREATE TABLE IF NOT EXISTS events (title TEXT, venue TEXT, day TEXT, price REAL, sold_tickets INTEGER, total_tickets INTEGER)";
        ...
        String createCartTable = "CREATE TABLE IF NOT EXISTS cart (username TEXT NOT NULL, event_title TEXT NOT NULL, event_venue TEXT NOT NULL, event_day TEXT NOT NULL, quantity INTEGER NOT NULL, total_price REAL NOT NULL, FOREIGN KEY (username) REFERENCES users(username))";
        String createIndexSql = "CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_event ON events (title, venue, day)";
    ...
    public void insertEvent(Event event) throws SQLException {
        String sql = "INSERT OR IGNORE INTO events (title, venue, day, price, sold_tickets, total_tickets) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public List<Event> getAllEvents() throws SQLException {
        List<Event> events = new ArrayList<>();
        String sql = "SELECT title, venue, day, price, sold_tickets, total_tickets FROM events";
        try (Statement stmt = connection.createStatement();
    ...
    private Event getEvent(String title, String venue, String day) throws SQLException {
        String sql = "SELECT * FROM events WHERE title = ? AND venue = ? AND day = ?";
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                return new Event(rs.getString("title"), rs.getString("venue"), rs.getString("day"),
                        rs.getDouble("price"), rs.getInt("sold_tickets"), rs.getInt("total_tickets"));
            }
        }
        return null;
    }
    ...
    public void updateEventTickets(Event event, int additionalTickets) throws SQLException {
        String sql = "UPDATE events SET sold_tickets = sold_tickets + ? WHERE title = ? AND venue = ? AND day = ?";
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
    ...
    public void addToCart(String username, Event event, int quantity) throws SQLException {
        ...
        String sql = "INSERT INTO cart (username, event_title, event_venue, event_day, quantity, total_price) " +
                "VALUES (?, ?, ?, ?, ?, ?)";
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            pstmt.setString(1, username);
            pstmt.setString(2, event.getTitle());
    ...
    public Cart getCartItems(String username) throws SQLException {
        Cart cart = new Cart();
        String sql = "SELECT event_title, event_venue, event_day, quantity, total_price FROM cart WHERE username = ?";
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            pstmt.setString(1, username);
            ResultSet rs = pstmt.executeQuery();
            ...
            while (rs.next()) {
                ...
                Event matchingEvent = allEvents.stream()
                        .filter(e -> e.getTitle().equals(title) && e.getVenue().equals(venue) && e.getDay().equals(day))
                        .findFirst()
                        .orElse(null);
